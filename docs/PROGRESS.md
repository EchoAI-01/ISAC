...
| R | 功能广度 (降级到 T 之后) | R3 ✅ 完成 (2026-08-16); R1/R2/R4/R5/R6/R7 未开始 | 补齐需求十二条仍缺的实现 + Q3-Q6/P3-P5 剩余接线。**2026-07-31 整组降级到 T 之后**(主干不可用时补功能广度无意义)。**R3 插件与 MCP 生态激活 (Q3) 已完成 (2026-08-16)**: 共享注册表 + AstrBot/MaiBot adapt 桥接 + MCPClient 生产接线 + CLI 工具 services 注入 (详见 §四 R3, 真机冒烟 `MCP server 已接入 server=echo tools=1`)。R1 多模态出入站闭环 (Q4) · R2 控制面与 SubAgent 收尾 (Q5+Q6) · R4 记忆完整性 (行话学习 + 中期记忆真实压缩 + P3 实体关系图) · R5 持久化与密钥安全 (Session 持久化 + SecretStore) · R6 企业化激活 (P5 剩余) · R7 集成测试补齐与发布准入 均未开始。定义见 DEVELOPMENT_PLAN §四 R |
