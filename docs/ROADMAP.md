...
ISAC 的能力分五个阶段推进。阶段 0/1 是"地基";阶段 1-4 各节点的**核心逻辑 + 单测已实现**,但多数**主链路未接线**(默认关闭、生产路径无调用点);为把这些能力接入同一主链路协同激活,新增 **阶段 5(P 节点)**。
...
**关键路径**: 拟人化地基 L1 → L2(Wait 闭环)是最优先的后续开发,因为 wait/主动/打断/debounce 一切拟人行为都以 ConversationRuntime 为地基。阶段 2/3/4 之间相对独立,可按业务优先级并行排期。**当前关键路径已转移**:L1-L5 等核心实现已完成,下一步是 **阶段 5 P 节点**把 `[~]` 能力接入主链路 —— P0(消息并发化)→ P1(拟人化激活)最优先,P2-P5 可并行。
