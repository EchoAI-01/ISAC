...
>
> 最近更新: 2026-08-15(前后端分离决策, 后端先行, 见阶段 9 与 DEVELOPMENT_PLAN §四 FE; T 开箱可用轮 T1/T2/T4 已完成 2026-08-04)
...
| **阶段 7** | **开箱可用 (最高优先级)** | T1-T7 | 🟡 **T1/T2/T4 已完成 (2026-08-04)**: 开箱能对话 (私聊无条件触发 + 未回复可观测 + 占位 key 检测)、零配置启动 (默认配置内置)、错误可诊断 (中文提示 + /health + 日志台后端), 均附真机冒烟证据。**T3 按前后端分离重定义** (后端段见阶段 9); T5 真实 IM 验收 (需凭据)、T6 插件市场 (依赖 R3)、T7 分发运维未开始 |
| **阶段 8** | 功能广度 (原阶段 7) | R1-R7 | 🔜 2026-07-29 制定, **07-31 降级到 T 之后**(主干不可用时补功能广度无意义)。收敛 Q3-Q6/P3-P5 剩余 + 需求缺口(行话学习/中期记忆/Session 持久化/密钥安全)+ 集成测试与发布准入。其中 **R3 是 T6 的前置** |
| **阶段 9** | **前后端分离 (后端先行)** | FE0/FE1/T3-backend + F1-F4 | 🔜 **2026-08-15 制定** (ADR-012)。后端纯 API 化: FE0 API 契约冻结 (OpenAPI) → FE1 CORS/跨源认证/内置 WebUI 静态托管降级 → T3-backend 控制面开箱 (control 默认开 + 首登强制设密码 API + 配置 Schema 端点); 前端独立项目 F1-F4 在 API 基线冻结后启动。定义见 DEVELOPMENT_PLAN §四 FE |
...
| **M-T2 可部署可管理** | 打开浏览器就能管理, 出错知道去哪修 | + **T3 (前后端分离: FE0/FE1/T3-backend + F1) + T4**: 控制面默认开 + setup/auth API + 首登向导;401/429 可操作中文提示 (✅) + `/health` (✅) + 实时日志台后端 (✅)。T4 后端段已完成, 剩 FE/T3 后端支撑与前端轨道 |
| **M-T3 可接入真实 IM** | 真实账号收发, 不只是单测 | + **T5**:OneBot/NapCat 真机跑通 + 各平台连接状态回显 + 飞书/QQ官方/企业微信按凭据逐个联调(此前从未真机验证)。需用户提供凭据与联调窗口 |
| **M-T4 生态可扩展** | 插件能装、能用、免重启 | + **R3 + T6**:插件桥接激活(否则装了也不触发)+ 市场一键安装 + 热重载 + 失败插件可重试 |
| **M-GA v1.0 正式版** | `REQUIREMENTS.md` 十二条全覆盖 + 可分发可长跑, 定位由 Alpha 升为可用版本 | + **T7**(compose 一键 + pip 安装 + 配置自动迁移 + 备份 + **24h soak** + 快速开始 5 分钟跑通)+ **R1/R2/R4/R5/R6**(多模态 / 控制面与 SubAgent / 记忆完整性 / 持久化与密钥 / 企业化)+ **R7**(P3/P4/P5 集成测试现全缺 + 每个 hook 有真实触发者驱动的测试 + I 节点升 100% + 发布清单七段 + 十二条逐条取证)。约 13-16 个开发轮(累计) |
