...
def test_control_enabled_without_auth_logs_critical(rec_logger: _RecordingLogger) -> None:
    # T3: setup_enabled 默认 True (首登强制设密码, 视为有认证保护, 不告警);
    # 仅"启用 control 但既无凭证又无 setup"才 CRITICAL, 故此处显式 setup_enabled=False。
    validate_config({"control": {"enabled": True, "api_token": "", "tokens": [], "setup_enabled": False}})
...
def test_control_null_fields_do_not_suppress_missing_auth_warning(
    ...
    # T3: setup_enabled 默认 True 不告警; 显式 False 才测"无凭证无 setup 仍 CRITICAL"。
    validate_config({"control": {"enabled": True, "api_token": None, "tokens": None, "setup_enabled": False}})
