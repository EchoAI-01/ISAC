...
- is_setup_required 只看自身 hash 是否存在; 是否"整体需要 setup"由 server 结合
  api_token/tokens/env 判定 (已配凭证时即使 setup_state 空也不强制 setup)。
"""
...
class SetupManager:
    ...
    def __init__(self, state_path: str = "data/control/setup_state.json") -> None:
    ...
    def is_setup_required(self) -> bool:
        """是否尚未设置首登密码 (本 manager 自身维度)。"""
        return self._hash is None
